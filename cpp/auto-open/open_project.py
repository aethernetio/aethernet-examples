...
repo_urls = {
    "Aether": "https://github.com/aethernetio/aether-client-cpp.git",
...
class BaseRunner:
    def clone(self, dir: str, repo_url: str):
        if not os.path.exists(dir):
            ...
            print(
                "The repository {} has been successfully cloned in {}".format(
                    repo_url, dir
                )
            )
    ...
    def cmake_config(self, project_dir: str, build_dir: str, *cmake_opts):
        ...
        cmake_cmd = self.get_cmake_cmd(
            "-S " + project_dir, "-B " + build_dir, *cmake_opts
        )
...
class LinuxRunner(BaseRunner):
    ...
    def run_vscode(self, path) -> subprocess.Popen:
        return subprocess.Popen(
            ["code", path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
class WindowsRunner(BaseRunner):
    def run_arduino_ide(self, sketch_path) -> subprocess.Popen:
        arduino_path = os.path.expanduser(
            "~/AppData/Local/Programs/arduino-ide/Arduino IDE.exe"
        )
        if not os.path.exists(arduino_path):
            arduino_path = "Arduino IDE.exe"
        print("cmd", [arduino_path, sketch_path])
        return subprocess.Popen(
            [arduino_path, sketch_path],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
    ...
    def run_vscode(self, path) -> subprocess.Popen:
        return subprocess.Popen(
            ["code.cmd", path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
class MacosRunner(BaseRunner):
    def run_arduino_ide(self, sketch_path) -> subprocess.Popen:
        return subprocess.Popen(
            ["open", "-a", "Arduino IDE.app", sketch_path],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
    ...
    def run_vscode(self, path) -> subprocess.Popen:
        return subprocess.Popen(
            ["open", "-a", "Visual Studio Code.app", path],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
...
class ArduinoRunner(IdeRunner):
    ...
    def run(self, project_dir: str, platform: str):
        ...
        sketch_path = os.path.join(
            project_dir, "arduino", project_name, project_dir + ".ino"
        )
        print(sketch_path)
...
class VSCodeWorkspaceRunner(IdeRunner):
    ...
    def run(self, project_dir: str, platform: str):
        workspace_path = os.path.join(
            project_dir, platform, platform + ".code-workspace"
        )
...
class CmakeBuilderRunner(BuilderRunner):
    ...
    def run(self, target_path: str):
        ...
        print(f"Running [{target_path}] in [{parent_dir}]")
        subprocess.run([target_path], check=True, cwd=parent_dir)
...
class ProjectOpener:
    ...
    def _prepare_config_file(self):
        if self._platform != "cmake":
            assert not "WifiAdapter" in self._conf, (
                f"Self wifi ssid and password required for {self._platform} platform"
            )
        self._config_writer.write(
            os.path.join("preregistered", "project_config.ini.in"),
            os.path.join("build-registrator", "project_config.ini"),
            self._conf,
        )
    ...
    def _prepare_project_config(self):
        ...
        self._config_writer.write(
            os.path.join("selfregistered", "project_config.h.in"),
            os.path.join("selfregistered", "project_config.h"),
            self._conf,
        )
    ...
    def _get_registrator(self):
        ...
        self._runner.cmake_config(
            os.path.join("aether-client-cpp", "tools", "registrator"),
            "build-registrator",
            "-DCMAKE_BUILD_TYPE=Release",
            "-DAE_DISTILLATION=On",
            "-DUSER_CONFIG=./preregistered/user_config.h",
        )
        # build registrator
        target_binary = self._runner.cmake_build(
            "build-registrator", "aether-registrator"
        )
    ...
    def _register(self, reg_tool: str):
        subprocess.run(
            [
                reg_tool,
                os.path.join("build-registrator", "project_config.ini"),
                os.path.join("preregistered", "registered_state.h"),
            ],
            check=True,
        )
    ...
    def _build_and_run(self):
        if not self._builder_runner:
            return
        print(
            "Build and run project {} for platform {}".format(
                self._project, self._platform
            )
        )
        target_path = ""
        if self._project == "preregistered":
            target_path = self._builder_runner.build(
                "preregistered", "preregistered", self._utm_id
            )
        elif self._project == "selfregistered":
            target_path = self._builder_runner.build(
                "selfregistered", "selfregistered", self._utm_id
            )
        if not target_path:
...
def open_project(project, platform, ide, wifi_ssid, wifi_pass, utm_id):
    print(
        "open project with parameters: [{}], [{}], [{}], [{}], [{}] [{}]!".format(
            project, platform, ide, wifi_ssid, wifi_pass, utm_id
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-project",
        type=str,
        help="Project to open {} '{}' - is default".format(
            known_projects, known_projects[0]
        ),
    )
    parser.add_argument(
        "-platform",
        type=str,
        help="Platform type (Arduino, cmake, platformio, esp-idf)",
    )
    ...
    if args.clean:
        ...
        open_project(
            args.project, args.platform, args.ide, args.ssid, args.password, args.utm_id
        )
