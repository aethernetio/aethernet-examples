...
constexpr auto kParentId =
    ae::Uid::FromString("3ac93165-3d37-4970-87a6-fa4ee27744e4");

constexpr ae::SafeStreamConfig kSafeStreamConfig{
    AE_SAFE_STREAM_CAPACITY / 2 - 1,  // window_size
    AE_SAFE_STREAM_CAPACITY / 2 - 1,  // max_data_size
    10,                               // max_repeat_count
...
void BobMeetAlice(ae::Client::ptr const& alice_client,
  ...
  context->bob_stream = ae::make_unique<ae::P2pSafeStream>(
      *context->aether_app, kSafeStreamConfig,
      std::make_shared<ae::P2pStream>(
          *context->aether_app, bob_client.Load(), alice_client->uid(),
          bob_client->message_stream_manager().CreatePort(
              alice_client->uid())));
  auto bob_say = std::string_view{"Hello"};
  context->bob_stream->Write({std::begin(bob_say), std::end(bob_say)})
      .status_event()
      .Subscribe([&](auto const status) {
  ...
  context->alice_stream = ae::make_unique<ae::P2pSafeStream>(
      *context->aether_app, kSafeStreamConfig,
      std::make_shared<ae::P2pStream>(
          *context->aether_app, alice_client.Load(), bob_client->uid(),
          alice_client->message_stream_manager().CreatePort(
              bob_client->uid())));
...
void setup() {
  ...
  context->aether_app->aether()
      ->SelectClient(kParentId, "Alice")
      .result_event()
      .Subscribe([&](auto const& alice_res) {
        if (alice_res) {
          context->aether_app->aether()
              ->SelectClient(kParentId, "Bob")
              .result_event()
              .Subscribe([&, alice = alice_res.value()](auto const& bob_res) {
                if (bob_res) {
                  BobMeetAlice(alice, bob_res.value());
                } else {
                  context->aether_app->Exit(1);
                }
              });
  ...
  // setup timeout
  context->aether_app->aether()->task_scheduler->DelayedTask(
      [&]() {
...
void app_main(void) {
  ...
  if (err != 0) {
    ESP_LOGE("Preregistered", "Reconfigure WDT is failed!");
  }
