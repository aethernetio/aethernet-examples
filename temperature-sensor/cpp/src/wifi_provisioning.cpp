  ...
  while (1) {
    switch (current_led_mode) {
      ...
      case LedMode::kClientConnected:
        SetLed(true, 255, 165, 0);
        vTaskDelay(pdMS_TO_TICKS(100));
        break;
      case LedMode::kConnecting:
        SetLed(true, 200, 200, 200);
        vTaskDelay(pdMS_TO_TICKS(100));
        SetLed(false, 0, 0, 0);
        vTaskDelay(pdMS_TO_TICKS(100));
        break;
 ...
 private:
  static esp_err_t IndexHandler(httpd_req_t* req) {
    ...
    snprintf(html, sizeof(html),
             ...
             "input,select,button{width:100%%;padding:15px;margin:10px "
             "0;border-radius:12px;font-size:16px;box-sizing:border-box;border:"
             "1px solid #333;background:#111;color:#fff;}"
             "button{background:#008cff;border:none;font-weight:bold;}</"
             "style></head>"
             "<body><h1>Step 3: WiFi</h1><p>Device ID: %s</p>"
             "<form action='/save' method='POST'><select name='s' "
             "id='s'><option>Scanning...</option></select>"
             "<input type='text' name='p' placeholder='WiFi Password' "
             "required><button type='submit'>CONNECT DEVICE</button></form>"
             "<script>fetch('/scan').then(r=>r.json()).then(d=>{let "
             "s=document.getElementById('s');s.innerHTML='';d.forEach(n=>{let "
             "o=document.createElement('option');o.value=n;o.text=n;s."
             "appendChild(o);});});</script></body></html>",
             device_uuid);
  ...
  static esp_err_t SaveHandler(httpd_req_t* req) {
    ...
    snprintf(redir, sizeof(redir),
             "https://aethernet.io/wizard.html?id=%s&status=checking",
             device_uuid);
  ...
  void ScanForNetworks() {
    ...
    for (int i = 0; i < ap_num; i++) {
      offset +=
          snprintf(cached_wifi_json_.data() + offset, 1024 - offset, "\"%s\"%s",
                   (char*)records[i].ssid, (i == ap_num - 1) ? "" : ",");
    }
  ...
  void RunServer() {
    ...
    if (httpd_start(&server_, &config) == ESP_OK) {
      httpd_uri_t i = {"/", HTTP_GET, IndexHandler, nullptr},
                  sc = {"/scan", HTTP_GET, ScanHandler, this},
                  sa = {"/save", HTTP_POST, SaveHandler, this};
...
static void WifiEventHandler(void* arg, esp_event_base_t base, int32_t id,
                             void* data) {
  bool btn_active = (current_led_mode == LedMode::kHoldSlow ||
                     current_led_mode == LedMode::kHoldFast ||
                     current_led_mode == LedMode::kResetting);
